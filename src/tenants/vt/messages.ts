...
export const copy = {
  ...
  about: {
    title: "Sobre mí",
    metaDescription: "Quién escribe VT Security, y dónde encontrarme.",
    paragraphs: [
      "Vivo en Rosario. Trabajo en ciberseguridad, sobre todo seguridad en la nube y cumplimiento, con bastante backend alrededor. Antes fui analista de seguridad, y ayudante de cátedra de Arquitectura de Computadoras.",
      "Hago videos en YouTube, en español. Este blog es donde escribo el mecanismo: por qué una key sigue viva después de que borraste el archivo, o qué mira el sistema cuando pedís un permiso.",
      "Escribo sobre lo que voy aprendiendo, no sobre lo que ya domino. Cuando algo lo probé, lo digo. Cuando no lo probé, también.",
      "Si encontrás un error, escribime por donde te quede más cómodo. Lo corrijo con una nota al pie que dice qué estaba mal.",
    ],
    profilesTitle: "Dónde encontrarme",
    profiles: {
      portfolio: "Portfolio",
      youtube: "YouTube",
      discord: "Discord",
    ...
    labsBefore: "Los labs para practicar lo que sale acá están en ",
    labsLink: "Open Security Labs",
    labsAfter: ".",
  },
  home: {
  ...
  figures: {
    ...
    bindScope: {
      bindLabel: "Escucha en:",
      bind: "Dirección en la que escucha",
      machine: "tu máquina",
      process: "python3 -m http.server",
      files: [".env", "app.js"],
      envFile: ".env",
      browser: "tu navegador",
      lan: ["otra máquina de tu red", "otra máquina"],
      net: "internet",
      router: "router",
      yourNet: "tu red",
      port: (addr: string) => `${addr}:3000`,
      chips: {
        leakWide: "OPENAI_API_KEY=sk-…",
        leakNarrow: ".env descargado",
        refused: "conexión rechazada",
      },
      play: "Mandar pedidos",
      pause: "Pausar",
      tallies: {
        served: (n: number) => `${n} ${n === 1 ? "respuesta" : "respuestas"}`,
        leaked: (n: number) => `se llevó el .env ${n} ${n === 1 ? "vez" : "veces"}`,
        refused: (n: number) => `${n} ${n === 1 ? "rechazado" : "rechazados"}`,
        dropped: (n: number) => `${n} ${n === 1 ? "cortado" : "cortados"}`,
      },
      results: {
        open: "<bad>Cualquiera en tu red llega al puerto y se baja el .env.</bad> Desde internet no, mientras el router no lo deje pasar.",
        local: "Solo tu navegador llega. La otra máquina recibe <code>Couldn't connect to server</code>.",
      },
    },
    miniBind: {
      machine: "tu máquina",
      lan: "otra máquina",
      file: ".env",
      noteWide: "En 0.0.0.0 el puerto le contesta a toda tu red. En 127.0.0.1, solo a tu máquina.",
      noteNarrow: "0.0.0.0 le contesta a toda tu red",
    },
    termVsKill: {
      signals: "Señales",
      buttons: { term: "kill", kill: "kill -9" },
      names: { term: "TERM", kill: "KILL" },
      inlet: "señal",
      client: "un cliente",
      app: "app",
      pid: "PID 4321",
      disk: "datos.db",
      lock: "app.pid",
      memory: (n: number) => `${n} ${n === 1 ? "cambio" : "cambios"} en memoria`,
      saved: (n: number) => `${n} ${n === 1 ? "cambio guardado" : "cambios guardados"}`,
      lost: (n: number) => `${n} ${n === 1 ? "cambio perdido" : "cambios perdidos"}`,
      chips: {
        draining: "cerrando…",
        exited: "salió solo",
        killed: "el kernel lo sacó",
        stale: "ya está corriendo (PID 4321)",
      },
      play: "Mandar pedidos",
      pause: "Pausar",
      again: "Levantar de nuevo",
      counters: { served: "respuestas", cut: "cortadas", refused: "rechazadas" },
      results: {
        running: "Atiende pedidos y cada tanto guarda en el disco. Mandale una señal.",
        draining: "<code>TERM</code>: deja de aceptar pedidos nuevos y termina los que tenía.",
        exited: "Terminó lo que tenía, guardó todo y borró <code>app.pid</code>. Lo que llega ahora rebota: no hay nadie escuchando.",
        /** Solo nombra lo que de verdad pasó: con 0 cambios en memoria no dice "0 cambios sin guardar" */
        killed: (cut: number, lost: number) => {
          const parts = [
            cut > 0 ? `${cut === 1 ? "un pedido" : `${cut} pedidos`} a medio atender` : "",
            lost > 0 ? `${lost === 1 ? "un cambio" : `${lost} cambios`} sin guardar` : "",
          ].filter(Boolean);
          const what = parts.length ? `con ${parts.join(" y ")}` : "en el acto";
          return `<bad>El kernel lo sacó ${what}.</bad> Y <code>app.pid</code> quedó.`;
        },
      },
    },
    respawn: {
      unit: "systemd --user",
      watches: "vigila demo-web.service",
      ...
      port: ":3000",
      busy: (pid: number) => `ocupado por ${pid}`,
      free: "libre",
      client: "un cliente",
      actions: "Acciones",
      buttons: { kill: "kill -9", stop: "systemctl stop", start: "systemctl start" },
      play: "Mandar pedidos",
      pause: "Pausar",
      counters: { restarts: "reinicios", served: "respuestas", failed: "sin respuesta" },
      results: {
        running: "systemd vigila el servicio. Mandá pedidos y probá matar el proceso.",
        down: "El proceso murió. Lo que llega ahora rebota hasta que systemd lo levante.",
        respawned: (pid: number) => `Otro PID, un segundo después: <code>${pid}</code>. El puerto vuelve a contestar.`,
        stopped: "<code>systemctl stop</code> le avisa a systemd. Nadie lo relanza y todo lo que llega rebota.",
        started: (pid: number) => `Vuelve a arrancar, con otro PID: <code>${pid}</code>.`,
      },
    },
  },
};
